...
const loginWithEmailAndPass = asyncHandler(async (req, res) => {
    ...
    const jwt = user.jwt
    return res
        .cookie('token', jwt.token, { maxAge: process.env.MAX_AGE_FOR_TOKEN, httpOnly: true, sameSite: 'none', secure: true })
        .status(200)
        .json(user);
});
...
const getAllUsers = asyncHandler(async (req, res) => {
    ...
    const users = await Auth.find({ _id: { $ne: userId } }).select('-password -resetToken').skip(skip).limit(limit)
